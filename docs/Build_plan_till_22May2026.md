...
2. **Builds a knowledge graph** from each document at runtime.
3. **Decomposes** amended/changed clauses into distinct atomic obligations —
...
4. **Maps** each obligation against the other document (gap / coverage).
5. **Diffs** — produces a clause-level change log with suggested edits.
6. **Judges** — scores impact/priority, surfaces non-obvious downstream effects.
7. **Answers follow-up Q&A**, grounded in the graphs, with citations.
...
1. Read the actual Stage-1 codebase end to end. Produce
   ...
   this plan.
2. Read `docs/STAGE2_PLAN.md` (the prior checklist) and reconcile it with this
   document. This document supersedes it where they conflict.
3. Confirm environment: `gcloud` installed and authenticated, GCP project set,
   billing active, `stage-2` branch created off `main`.
4. Confirm/enable required GCP APIs: Vertex AI / Gemini, Document AI, Spanner,
   Cloud Run, Cloud Trace, plus whatever the Agents CLI needs.
5. Verify the credit situation is understood (the $500 challenge credit; the
...
6. Make the comparison-mode decision (§1.6). Record it in `docs/DECISIONS.md`.
7. Start `docs/DECISIONS.md` — an append-only log of every non-obvious choice,
...
2. Strengthen the data contracts in `app/models.py` as needed; keep them strict.
3. Expand `tests/eval/` well beyond 5 cases — cover fan-out depth, missing-
   coverage, contradiction, citation correctness, multi-hop Q&A.
4. Tune prompts against the evalset until quality is consistently high.
...
2. Build the ingestion front of the pipeline:
...
3. Implement `SpannerGraphBackend` against the existing `GroundingBackend` ABC.
   The agent chain from Phase 1 should run unchanged on top of it.
4. Keep `MockGroundingBackend` working — offline tests must still pass.
5. Handle the messy realities: large PDFs, OCR noise, scanned pages, tables,
...
2. **A2A interoperability** — expose the agent via the A2A protocol so it is
   discoverable/coordinatable. Research the minimum-viable correct A2A
   implementation; don't over-build.
3. **Minimal web UI** — upload two PDFs, show progress through the pipeline,
...
4. **Observability** — ensure Agent Observability / Cloud Trace shows the
   decompose→map→diff→judge reasoning trace. This visible trace is strong
   evidence for the Demo and Technical scores.
5. **Testing access** — make sure a judge can actually run it: a deployed URL,
...
2. **Architecture diagram** — the runtime GraphRAG pipeline, clean and clear.
3. **Written submission fields** — Problem, Solution, Technologies, Data
...
4. **Repo hygiene** — public, OSI license, clean README, one-command setup,
   no secrets, no `bbb` artifacts, no proprietary Curator detail.
5. **The 5 submission questions** — answer honestly from real build experience
...
6. **Submit on Devpost** with buffer before June 5, 5 PM PT. Do not wait for
   the last hour.
