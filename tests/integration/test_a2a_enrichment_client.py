"""Tests for the A2A bridge to the external A2A regulatory-corpus gateway.
...
One live test hard-skips when ``CURATOR_A2A_BEARER`` is unset; when set it
...
from app.tools.a2a_enrichment_client import (
    _coerce_result,
    is_enabled,
...
def test_disabled_when_no_bearer(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("CURATOR_A2A_BEARER", raising=False)
...
@pytest.mark.skipif(
    not os.environ.get("CURATOR_A2A_BEARER"),
    reason="CURATOR_A2A_BEARER not set — skipping live A2A bridge call",
)
