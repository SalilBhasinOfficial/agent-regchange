...
Inputs: the quantitative parameter changes (old → new risk weights, LTV
bands, capital charges, effective dates) diffed between the two frameworks,
the obligations extracted from the amendment, the coverage matches against
the bank's policies, and the suggested diffs.

When parameter changes are present, ground your assessment in them: cite the
specific risk-weight / capital movements that drive the priority, and state
the direction of the Pillar-1 CRAR impact (higher risk weights → higher RWA
→ lower CRAR, all else equal). Name the hardest effective date.
...
def _format_judge_prompt(
    ...
    matches: list[PolicyMatch],
    diffs: list[PolicyDiff],
    param_changes: list[ParameterChange] | None = None,
) -> str:
    ...
    param_text = "\n".join(
        f"- {p.parameter}"
        + (f" [{p.exposure_class}]" if p.exposure_class else "")
        + f": {p.old_value or 'n/a'} → {p.new_value} ({p.direction})"
        + (f"; effective {p.effective_date}" if p.effective_date else "")
        + (f"; {p.crar_impact}" if p.crar_impact else "")
        for p in (param_changes or [])
    )
    ...
    param_block = (
        f"Quantitative Parameter Changes (old → new):\n"
        f"==========================================\n"
        f"{param_text}\n\n"
        if param_text
        else ""
...
def _real_judge_single_shot(
    ...
    prompt = _format_judge_prompt(obligations, matches, diffs, param_changes)
...
def real_judge(
    ...
    if mode == "single":
        return _real_judge_single_shot(obligations, matches, diffs, param_changes)
