...
-- ---------------------------------------------------------------------------
-- pipeline_runs — persists FastAPI's in-memory _RUNS dict so /impact/<id>
-- survives container scale-to-zero. Written once per pipeline run when
-- the chain completes (or errors); never updated. The full AgentState
-- is serialised into state_json so the impact + Q&A views can render
-- without re-running the chain.
-- ---------------------------------------------------------------------------
CREATE TABLE pipeline_runs (
  pipeline_run_id STRING(64)   NOT NULL,
  ts              TIMESTAMP    NOT NULL OPTIONS (allow_commit_timestamp=true),
  amendment_id    STRING(MAX),
  status          STRING(16),                       -- "done" / "error"
  clauses_count   INT64,
  state_json      STRING(MAX),                      -- AgentState.model_dump_json() on success
  error           STRING(MAX),                      -- traceback head on failure
  total_cost_usd  FLOAT64,
  stats_json      STRING(MAX),                      -- perf stats: pages, per-stage timing, model
