...
rather than embedding any of its data layer. When ``CURATOR_A2A_BEARER`` is
...
_DEFAULT_ENDPOINT = ""  # disabled unless CURATOR_A2A_GATEWAY_URL is set
_DEFAULT_TOOL_NAME = "regulatory_deep_lookup"
_DEFAULT_TIMEOUT_S = 8.0


def _endpoint() -> str:
    return os.environ.get("CURATOR_A2A_GATEWAY_URL", _DEFAULT_ENDPOINT)


def _bearer() -> str | None:
    token = os.environ.get("CURATOR_A2A_BEARER")
    return token.strip() if token else None


def is_enabled() -> bool:
    """True only when BOTH the gateway URL and bearer are configured.

    The public repo ships with neither set, so every call is a silent
    no-op returning ``[]`` and the offline path stays green. Point
    ``CURATOR_A2A_GATEWAY_URL`` at any MCP/A2A regulatory-corpus gateway
    and set ``CURATOR_A2A_BEARER`` to enable enrichment.
    """
    return bool(_endpoint()) and _bearer() is not None
...
def regulatory_deep_lookup(query: str, k: int = 5) -> list[dict[str, Any]]:
    ...
    payload = {
        ...
        "method": "tools/call",
        "params": {
            "name": os.environ.get("CURATOR_A2A_TOOL_NAME", _DEFAULT_TOOL_NAME),
    ...
    try:
        ...
        logger.warning("a2a_enrichment_client: lookup failed (%s); returning empty.", exc)
        return []
