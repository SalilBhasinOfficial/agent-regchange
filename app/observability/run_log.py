...
def begin_pipeline_run() -> str:
    ...
    """
    pid = uuid.uuid4().hex
    bind_pipeline_run(pid)
    return pid


def bind_pipeline_run(pipeline_run_id: str) -> None:
    """Bind an EXISTING pipeline_run_id to the current scope.

    Use this from a worker thread that needs to inherit the pipeline id
    its parent already opened — calling :func:`begin_pipeline_run` in
    the worker would generate a NEW uuid and clobber the mirror, so
    every agent_runs row would point at an id the parent never knew
    (B2 bug, 2026-06-05).
    """
    global _pipeline_run_id_mirror
    _pipeline_run_id.set(pipeline_run_id)
    with _pipeline_lock:
        _pipeline_run_id_mirror = pipeline_run_id
...
def log_run(
    *,
    ...
    # Caller may pass an explicit pipeline_run_id (e.g. ThreadPoolExecutor
    # worker that captured it at submit time). Otherwise fall through to
    # the ContextVar→mirror lookup so threaded panel calls still tag with
    # the parent pipeline (B2 fix, 2026-06-05).
